...
- **Active Backlog**: `docs/specs/_product-backlog.md` (23 active items; Kind/Labels/Priority columns active 2026-05-06; Shipped/Cancelled rows split to _product-backlog-archive.md 2026-06-02 per backlog #8)
- **Spec Index** (shipped project specs at `docs/specs/`; lists shipped graduations only — draft/research specs are tracked in `docs/specs/_product-backlog.md` `Spec File` column, not here):
...
- **Branch `fix/ssot-drift-adr-index-backlog-count`** (quick-win, governance/SSoT) — Broader SSoT/doc-drift audit requested by user; verified every `current_state.md` claim against the source-of-truth files and fixed 3 confirmed drifts. Read-only diagnosis first (classification deferred), then governed quick-win remediation.
  ...
  - **Drift 2 (backlog count)**: Active Backlog said `(40 items)` — stale. Actual active backlog = 23 rows (21 Pending + 2 Shipped-not-yet-archived); 33 rows live in `_product-backlog-archive.md`. `40` was accurate pre-2026-06-02; the archive split (backlog #8) moved 33 rows to cold storage and dropped active to 23, but the SSoT count was never updated. → `23 active items` + archive-split note.
  ...
  - **Honest scope**: SSoT metadata correction only; no semantic rule/behavior change, no validator added. Also refreshed `Last Verified`→2026-06-09. **Deferred (out of scope, follow-up)**: archiving backlog rows #50/#56 (Shipped, still in active backlog) — touching `_product-backlog.md` routes to `/spec-intake`, kept separate.
  - **Evidence**: all 3 drifts grep-verified against ADR frontmatter + backlog row counts; SSoT writes via `guard_context_write.py` (optimistic-lock replace, expected-sha verified); `bash validate.sh` → pass=99 warn=9 fail=0 skip=2 (warn delta vs v1.4.1 baseline = this branch's own in-flight work log, none touch `current_state.md`). Implementation commit `f3ac21c`.
