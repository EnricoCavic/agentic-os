...
- **Classification Freeze**: Task category is locked after bootstrap. Silent downgrade is prohibited. Upgrade-only reclassification is allowed ONLY via explicit rollback to `CLASSIFIED`, then re-run the next required gate from there. **Reverse Transition (all platforms)**: If scope creep is detected mid-implement, the agent MUST stop, transition state back to `IMPLEMENTING → CLASSIFIED`, and surface the scope change to the user before continuing. This applies regardless of platform (Claude, Codex, Antigravity). Full state machine: `.agent/rules/state_machine.md`.
